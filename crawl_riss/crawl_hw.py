...
def get_total_papers_count(search_keyword, start_count=0):
    ...
    base_url = f'http://www.riss.kr/search/Search.do?isDetailSearch=N&searchGubun=true&viewYn=OP&query={search_keyword}\
...
&orderBy=&fsearchMethod=search&isFDetailSearch=N&sflag=1&searchQuery={search_keyword}\
&fsearchSort=&fsearchOrder=&limiterList=&limiterListText=&facetList=&facetListText=&fsearchDB=&resultKeyword={search_keyword}\
&pageNumber=1&p_year1=&p_year2=&dorg_storage=&mat_type=&mat_subtype=&fulltext_kind=&t_gubun=&learning_type=&language_code=&\
ccl_code=&language=&inside_outside=&fric_yn=&db_type=&image_yn=&regnm=&gubun=&kdc=&ttsUseYn='
...
def crawl_papers_hw(search_keyword, max_papers=1000):
...
ccl_code=&language=&inside_outside=&fric_yn=&db_type=&image_yn=&regnm=&gubun=&kdc=&ttsUseYn='
        ...
        for cont in contents:
            ...
            university.append(cont.find('p', class_="etc").find_all('span')[1].text.strip())
            year.append(cont.find('p', class_="etc").find_all('span')[2].text.strip())
            grad.append(cont.find('p', class_="etc").find_all('span')[3].text.strip())
    ...
    df = pd.DataFrame({
        ...
        'University': university[:papers_to_collect],
        'Year': year[:papers_to_collect],
        '석박사 구분': grad[:papers_to_collect],
    ...
    folder_name = search_keyword.replace(" ", "_")+"_학위논문"
